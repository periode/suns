...
const Entrypoint = (props: any) => {
  ...
  useEffect(() => {
    if (hasData.current === false) {
      fetchEntrypoint(params.id as string, session.token)
        .then((e: IEntrypoint) => {
          e.modules.sort(
            (a: IModule, b: IModule) => parseInt(a.ID) - parseInt(b.ID)
          );
          ...
          if (
            current.type === "intro" ||
            (current.type === "task" &&
              current.tasks[0].type === "prompts_input") ||
            (current.type === "final" &&
              e.current_module < e.modules.length - 1)
          )
    ...
    if (data.users.length > 0 && session.user.uuid !== "")
      for (let i = 0; i < data.users.length; i++) {
        ...
        if (u.uuid === session.user.uuid) {
          ...
          if (data.user_completed[i] === 1) setUserCompleted(true);
        }
      }
  ...
  const claimEntrypoint = async () => {
    const endpoint = new URL(
      `entrypoints/${data.uuid}/claim`,
      process.env.REACT_APP_API_URL
    );

    if (session.token === "") Navigate({ to: "/auth" });
    ...
    if (res.ok) {
      ...
      updated.modules.sort(
        (a: IModule, b: IModule) => parseInt(a.ID) - parseInt(b.ID)
      );
      setData(updated);
      if (updated.max_users > 1) setCanUserComplete(false);
    } else {
      setFetching(false);
  ...
  const completeModule = async (ep: IEntrypoint, session: ISession) => {
    ...
    progressModule(ep.uuid, session.token)
      .then((updated) => {
        ...
        else setUserCompleted(false); //-- we move on to the next module
  ...
  const parseModule = (index: number, ep: IEntrypoint) => {
    ...
    switch (mod.type) {
      case "intro":
        return <IntroModule airtable_key={ep.airtable_key} data={mod} />;
      case "task":
        return (
          <TaskModule
            index={index}
            ...
            setCanUserComplete={setCanUserComplete}
          />
        );
      case "final":
        return <PublicView entrypoint={ep} />;
      default:
        return (
          <>
        ...
        <EntrypointLayout
          ...
          module={
            <div className="w-full h-full p-4 overflow-y-scroll">
              ...
              ) : data.status === ENTRYPOINT_STATUS.EntrypointPending &&
                !isOwned ? (
                <IntroModule
                  airtable_key={data.airtable_key}
                  data={data.modules[0]}
                />
              ) : (
                getModule()
